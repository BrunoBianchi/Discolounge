  ...
  (function($) {
    $.fn.amsifySuggestags = function(options, method) {
      ...
      AmsifySuggestags.prototype = {
        ...
        createHTML: function() {
          var HTML =
            '<div class="' + this.classes.sTagsArea.substring(1) + '"></div>';
          this.selectors.sTagsArea = $(HTML).insertAfter(this.selector);
          var labelHTML =
            '<div class="' + this.classes.inputArea.substring(1) + '"></div>';
          this.selectors.inputArea = $(labelHTML).appendTo(
            this.selectors.sTagsArea
          );
  
          this.defaultLabel =
            $(this.selector).attr("placeholder") !== undefined
              ? $(this.selector).attr("placeholder")
              : this.defaultLabel;
          var sTagsInput =
            '<input type="text" class="' +
            this.classes.sTagsInput.substring(1) +
            '" placeholder="' +
            this.defaultLabel +
            '">';
          this.selectors.sTagsInput = $(sTagsInput)
            .appendTo(this.selectors.inputArea)
            .attr("autocomplete", "off");
  
          var listArea =
            '<div class="' + this.classes.listArea.substring(1) + '"></div>';
          this.selectors.listArea = $(listArea).appendTo(
            this.selectors.sTagsArea
          );
          $(this.selectors.listArea).width(
            $(this.selectors.sTagsArea).width() - 3
          );
        ...
        setEvents: function() {
          var _self = this;
          $(this.selectors.inputArea)
            .attr("style", $(this.selector).attr("style"))
            .addClass($(this.selector).attr("class"));
          $(this.selectors.sTagsInput).focus(function() {
            $(this)
              .closest(_self.classes.inputArea)
              .addClass(_self.classes.focus.substring(1));
          ...
          $(this.selectors.sTagsInput).blur(function() {
            $(this)
              .closest(_self.classes.inputArea)
              .removeClass(_self.classes.focus.substring(1));
          });
          $(this.selectors.sTagsInput).keyup(function(e) {
            ...
            if (keycode == "13" || keycode == "188") {
              ...
              if ($(this).hasClass(removeClass)) {
                $item = $(this)
                  .closest(_self.classes.inputArea)
                  .find(_self.classes.tagItem + ":last");
          ...
          $(window).resize(function() {
            $(_self.selectors.listArea).width(
              $(_self.selectors.sTagsArea).width() - 3
            );
          });
          $(this.selectors.sTagsArea).click(function() {
            $(_self.selectors.sTagsInput).focus();
          });
          $(this.selectors.listArea)
            .find(this.classes.listItem)
            .hover(
              function() {
                $(_self.selectors.listArea)
                  .find(_self.classes.listItem)
                  .removeClass("active");
        ...
        upDownSuggestion: function(value, type) {
          ...
          var isActive = false;
          $(this.selectors.listArea)
            .find(this.classes.listItem + ":visible")
            .each(function() {
              if ($(this).hasClass("active")) {
                $(this).removeClass("active");
                if (type == "up") {
                  $item = $(this).prevAll(
                    _self.classes.listItem + ":visible:first"
                  );
                } else {
                  $item = $(this).nextAll(
                    _self.classes.listItem + ":visible:first"
          ...
          if (!isActive) {
            ...
            $item = $(this.selectors.listArea).find(
              this.classes.listItem + ":visible:" + childItem
            );
            if ($item.length) {
              $item.addClass("active");
              $(_self.selectors.sTagsInput).val($item.data("val"));
        ...
        suggestWhiteList: function(value) {
          ...
          $(this.selectors.listArea)
            ...
            .each(function() {
              if (
                ...
                $.inArray($(this).attr("data-val"), _self.tagNames) === -1
              ) {
                $(this).show();
                found = true;
              } else {
          ...
          if (found) $(this.selectors.listArea).show();
          else $(this.selectors.listArea).hide();
        },
        ...
        createList: function() {
          ...
          $.each(settings.suggestions, function(index, item) {
            listHTML +=
              '<li class="' +
              _self.classes.listItem.substring(1) +
              '" data-val="' +
              item +
              '">' +
              item +
        ...
        addTag: function(value) {
          ...
          var html =
            '<span class="' +
            this.classes.tagItem.substring(1) +
            '" data-val="' +
            value +
            '">' +
            value +
          ...
          if (
            settings.tagLimit != -1 &&
            settings.tagLimit > 0 &&
            this.tagNames.length >= settings.tagLimit
          ) {
        ...
        removeTag: function(item, animate) {
          ...
          this.setInputValue();
          $(this.selector).trigger("suggestags.remove", [
            $(item).attr("data-val")
          ]);
        ...
        setIcon: function() {
          ...
          if (settings.type == "bootstrap") {
            ...
            return (
              '<i class="material-icons right ' + removeClass + '">clear</i>'
            );
          } else {
        ...
        fixCSS: function() {
          if (settings.type == "amsify") {
            $(this.selectors.inputArea)
              .addClass(this.classes.inputAreaDef.substring(1))
              .css({ padding: "5px 5px" });
          } else if (settings.type == "materialize") {
            $(this.selectors.inputArea)
              .addClass(this.classes.inputAreaDef.substring(1))
              .css({ height: "auto", padding: "5px 5px" });
